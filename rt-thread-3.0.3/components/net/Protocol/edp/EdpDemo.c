...
/**
  ...
  *            但是由于串口数据参杂着8266模块的上报信息，所以在进行EDP解析
  *            之前，需要将串口数据进行解析
  */
void Recv_Thread_Func(void)
{
            ...
            switch (mtype)
            {
                ...
                case CMDREQ:
                    if (UnpackCmdReq(pkg, &cmdid, &cmdid_len, &cmd_req,&cmd_req_len) == 0)
                    {
                        ...
                        edpPkt = PacketCmdResp(cmdid, cmdid_len, cmd_resp, cmd_resp_len);
#ifdef _ENCRYPT
                        if (g_is_encrypt)
                        {
                            SymmEncrypt(edpPkt);
                        ...
                        /* 在这里添加需要实现的控制功能代码 */
                        if(strstr(cmd_req, "led,") != 0)
                        {
                            key_value = atoi(strstr(cmd_req, ",") + 1);
                            if(key_value == 1)
                            {
                                printf("!!!!led ON !!!!!!\r\n");
                                LED_7_ON;
                            ...
                            sprintf(buf, ",;%s", cmd_req);
                            printf("response %s\r\n", buf);
                            
                            /* EDP type=5 上传数据 */
                            edpPkt = PacketSavedataSimpleString(NULL, buf);
                            printf("send save data to server, bytes: %d\r\n", edpPkt->_write_pos);
                            sendPkt(&edpPkt);		//非透传模式发送
                        }
